 ...
 */
int main_test_loop(char * input_gesamt, char * input_testfile, int fail_case);
...
int main_test_loop(char * input_gesamt, char * input_testfile, int fail_case) {
    ...
    switch(pid) {
        ...
        case 0:
            ...
            char * path_string = "../../../abgabe/gesamt/gesamt";
            execlp(path_string, "gesamt", NULL);
    ...
    ret = test_loop(parent_child_pipe[1], child_parent_pipe[0], input_gesamt, output);
...
int execute_test_cases() {
    ...
    // ONLY-GESAMT-TESTS
    {
        ...
        test_cases_successful += main_test_loop(
                "f(x) return g(x); end; g(x) return x; end;", 
                "long fc = f(1); if(fc == 1) { exit(0);} exit(1);\n", SHOULD_WORK);
    }
    {
        // Test 52:
        test_cases_successful += main_test_loop(
                "f(x) return g(x,2,4); end; g(x,y,z) return (x+y+z); end;", 
                "long fc = f(1); if(fc == 7) { exit(0);} exit(1);\n", SHOULD_WORK);
    }
    {
        // Test 53:
        test_cases_successful += main_test_loop(
                "f(x) return g(x+1); end; g(x) return c(x*2); end; c(x) return x; end;", 
                "long fc = f(1); if(fc == 4) { exit(0);} exit(1);\n", SHOULD_WORK);
    }
    {
        // Test 54:
        test_cases_successful += main_test_loop(
                "f(x) return g{x}@(1); end; g{x}(y) return (x+y); end;", 
                "long fc = f(1); if(fc == 2) { exit(0);} exit(1);\n", SHOULD_WORK);
    }
    {
        // Test 55:
        test_cases_successful += main_test_loop(
                "f(x,y) return g{x,y}@(1,3); end; g{x,y}(z,k) return (x+y+z+k); end;", 
                "long fc = f(5,7); if(fc == 16) { exit(0);} exit(1);\n", SHOULD_WORK);
    }
    {
        // Test 56:
        test_cases_successful += main_test_loop(
